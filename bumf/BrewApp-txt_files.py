...
def dict_from_file(filepath, dict_name):
    ...
    try:
        file_list = [item.strip() for item in file_list]
        for item in file_list:
            list_pair = item.split(", ")
            dict_name[list_pair[0]] = list_pair[1]
        file_var.close()
        return dict_name
    except FileNotFoundError as fnfe:
        print("Unable to open file:" + str(fnfe))
...
dict_from_file("preferences.txt", preferences)
...
def get_favourites():
    tabulate_dict("favourites", preferences)
...
def round_menu():
    ...
    while True:
        ...
        if command == "1":
            ...
            new_round.add_preferences()
            print("\nPreferences added to round.")
            print(round_menu_text)
        elif command == "3":
            tabulate_dict(new_round.round_name, new_round.orders)
            print(round_menu_text)
        elif command == "4":
...
[1] Add to round
[2] Populate with all preferences
[3] Print round
[4] Return to menu
"""
...
def main_menu():
    ...
    if command == GET_PEOPLE_CMD:
        ...
        save_dict_to_file("preferences.txt", preferences)
